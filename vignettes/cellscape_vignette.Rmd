...
To obtain single cell targeted mutation data, extract allele counts and compute variant allele frequencies (variant_count / total_count).

To obtain single cell copy number data, there are two main approaches. The first approach is to use methods based on Hidden Markov Models (e.g. HMMcopy - http://compbio.bccrc.ca/software/hmmcopy/). The second approach is to use methods based on circular binary segmentation (CBS).
